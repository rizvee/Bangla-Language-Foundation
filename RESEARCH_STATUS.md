...
## 1. Project Phase
- **Current Phase**: Phase 2A.2b — Blinded Review Protocol & IAA Methodology Hardening (Complete)
- **Milestone State**: Phase 1A-1D, 2A, 2A.1, 2A.2, and 2A.2b Operational
- **Primary Branch**: `main`
...
|---|---|---|
| **Schemas Authoring** | 16 schemas (`v0.1-draft-2.0`) | Utterance, Sentence Family, Source, Synthetic Provenance, Linguistic Evidence, Linguistic Claim, Linguistic Rule, Linguistic Example, Inflectional Paradigm, Linguistic Construction, Complex Predicate, Dialogue Act, Semantic Frame, Corpus Attestation, Human Review Decision, Review Adjudication |
...
| **Blinded Reviewer Packages** | 2 rater packs | Randomly permuted candidate sets in `data/review_queue/blinded_packs/` (`REV-LINGUIST-01`, `REV-NATIVE-02`) |
...
| **Automated Tests** | 81 unit tests | 100% passing across 15 test suites |
...
- **Blinded Review Generator & Randomizer**: Implemented in `scripts/generate_blinded_pilot.py` and secret permutation mapping `data/review_queue/pilot_40_randomization_mapping.json`.
- **Human Review Framework & IAA Tooling**: Implemented in `schemas/v0_1/human_review_decision.schema.json`, `schemas/v0_1/review_adjudication.schema.json`, `src/blf/quality/iaa.py`, and `scripts/compute_iaa.py`.
...
1. **Stage 1 Human Evaluation Execution**: The blinded packages in `data/review_queue/blinded_packs/` await evaluation by native linguists prior to Phase 3 Gold seed scaling.
